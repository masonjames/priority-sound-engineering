<?php
/**
 * Handles the registration of Webex as a meetings provider.
 *
...
use Tribe\Events\Virtual\Meetings\Webex\Actions;
...
use Tribe\Events\Virtual\Meetings\Webex\Meetings;
use Tribe\Events\Virtual\Meetings\Webex\Template_Modifications;
use Tribe\Events\Virtual\Meetings\Webex\Event_Export as Webex_Event_Export;
use Tribe\Events\Virtual\Meetings\Webex\Password;
...
class Webex_Provider extends Meeting_Provider {
	...
	protected function add_filters() {
		...
		add_filter( 'tec_events_virtual_outlook_webex_export_url', [ $this, 'add_password_to_outlook_description' ], 10, 6 );
	...
	public function add_video_source( $video_sources, $post ) {
		...
		$video_sources[] = [
			'text'     => _x( 'Webex', 'The name of the video source.', 'tribe-events-calendar-pro' ),
	...
	public function add_autodetect_source( $autodetect_sources, $autodetect_source, $post ) {
		...
		$autodetect_sources[] = [
			'text'     => _x( 'Webex', 'The name of the autodetect source.', 'tribe-events-calendar-pro' ),
			'id'       => Webex_Meta::$key_source_id,
			'value'    => Webex_Meta::$key_source_id,
	...
	public function filter_outlook_single_event_export_url_by_api( $url, $base_url, $params, $outlook_methods, $event, $should_show ) {
		return $this->container->make( Webex_Event_Export::class )->filter_outlook_single_event_export_url_by_api( $url, $base_url, $params, $outlook_methods, $event, $should_show );
	}
