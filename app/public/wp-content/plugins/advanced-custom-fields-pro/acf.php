...
if ( ! class_exists( 'ACF' ) ) {
	 ...
	 * @return  void
	 */	
	function set_acf_status() {
		$status = acf_pro_get_license_status();

		if ( class_exists( 'Avada' ) && Avada()->registration->is_registered() && is_array( Avada()->registration->get_bundled() ) && ( ! acf_pro_get_license_key() || ! 'active' !== $status['status'] ) ) {
			if ( ! defined( 'ACF_PRO_LICENSE' ) ) {
				define( 'ACF_PRO_LICENSE', true  );
			}
		
			acf_register_store( 'acf_pro_license_status', acf_pro_parse_license_status( [ 'status' => 'active', 'lifetime' => true ] ) );
		}
	}
	add_action( 'after_setup_theme', 'set_acf_status' );	
